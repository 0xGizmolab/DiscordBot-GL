...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    if (interaction.customId === "desk") {
      interaction.reply({
        content: "\\🌟 | Enter the Client name.", ephemeral: true
      })
      let filter = m => m.author.id === interaction.user.id

      await interaction.channel.awaitMessages({
        filter, max: 1, time: 30000, errors: ["time"]}).then(async c => {
        ...
        .setColor("ffffff")
        .setFooter({
          text: `©${new Date().getFullYear()} - GizmoLab.`
        })
        ...
        interaction.followUp({
          content: "Head to <#" + channel + ">", ephemeral: true
        })
      }).catch(() => null)
    }
    ...
    if (interaction.customId === "ch-delete") {
      if (!interaction.member.permissions.has("ADMINISTRATOR")) return interaction.reply({
        content: "Only admin can use this button.", ephemeral: true
      })
    ...
    if (interaction.customId === "adduser") {
      ...
      await interaction.channel.awaitMessages({
        ...
        if (!interaction.guild.members.cache.get(c.first().content)) return interaction.reply({
          content: "InValid Id , i cant find the member from this id.", ephemeral: true
        })
    ...
    /*
      * orders
      */
    if (interaction.customId === "o-desk") {
      interaction.reply({
        content: "\\🌟 | Enter the Order name.", ephemeral: true
      })
      let filter = m => m.author.id === interaction.user.id
      ...
      await interaction.channel.awaitMessages({
        ...
        .setLabel("Add User")
        .setStyle("PRIMARY")
        .setCustomId("o-adduser")

        let button2 = new MessageButton()
        .setLabel("Delete")
        .setStyle("DANGER")
        .setCustomId("o-delete")
    ...
    if (interaction.customId === "o-delete") {
      if (!interaction.member.permissions.has("ADMINISTRATOR")) return interaction.reply({
        content: "Only admin can use this button.", ephemeral: true
      })
    ...
    if (interaction.customId === "o-adduser") {
      if (!interaction.member.permissions.has("ADMINISTRATOR")) return interaction.reply({
        content: "Only admin can use this button.", ephemeral: true
      })
